"""
persona robustness in run_inference.py
"""
...
@torch.no_grad()
def get_toxicity_result(args, tokenizer, llm, input_question_list, prompt_list, persona):
    save_folder = f"logs/{args.benchmark}/robustness/{persona}/"
    os.makedirs(save_folder, exist_ok=True)
    if args.external:
        if args.wo_unbiased_instruc:
            save_file = save_folder + "extrinsic.json"
    ...
    if os.path.exists(save_file):
        with open(save_file,'r') as reader:
            hs_probing_list = json.load(reader)
    print(f"READ SAMPLE LENGTH: {len(hs_probing_list)}")
    persona_prompt = copy.deepcopy(persona_prompt).replace("#PERSONA", persona) + "</s>\n"
    ...
    for q_idx, question_ in tqdm(enumerate(input_question_list)):
        ...
        for idx, prompt in enumerate(prompt_list):
            ...
            if args.persona: query = persona_prompt + query
            if args.wo_unbiased_instruc:
        ...
        if count == args.num_samples:
            print(f"total {args.num_samples} done!")
            break
    return len(hs_probing_list)
...
if __name__ == "__main__":
    ...
    if "bbq" in args.benchmark:
        biases = ["sexualorientation", "disability", "physical", "religion"]
    ...
    if "toxicity" in args.benchmark:
        ...
        for persona in personas:
            ...
            with open(f'{save_folder}{log_name}', 'w') as log_file:
                sys.stdout = log_file
            print(f"PERSONA: {persona}|LOG PATH: {save_folder}|SAVE PATH: {save_folder}")
            if args.external:
            ...
            probing_result_len = get_toxicity_result(args,tokenizer,llm, question_list, prompt_list, persona)
            print(f"role {persona}, {probing_result_len} samples done")
