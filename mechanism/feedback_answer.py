    ...
    for question_idx, question_ in enumerate(input_question_list[:]):
        ...
        for idx, prompt_ in enumerate(prompt_list):
            ...
            if args.extrinsic and args.request_feedback == 1:
                if round_idx % 2 != 0: continue #only request feedback to answer
    ...
    if "bbq" in args.benchmark:
        ...
        if args.extrinsic and args.cot:
            prompt_list = [realtoxicity_extrinsic_cot, realtoxicity_cot_round2,
                                bbq_selfcorrect_extrinsic_cot, realtoxicity_cot_round2, realtoxicity_extrinsic_cot_refine, realtoxicity_cot_round2]
            save_file = f"./logs/realtoxicity/debug/extrinsic_cot_{args.bias}.json"
            with open(save_file,'w') as writer: 
                            result_list = get_bbq_result(args,tokenizer,llm, question_list, prompt_list)
                            json.dump(result_list,writer)
                            print(f"save to {save_file}")
